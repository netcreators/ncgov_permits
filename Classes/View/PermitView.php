...
class PermitView extends BaseView
{
        ...
        if (\tx_nclib::isLoopable($record['objectaddresses'])) {
            foreach ($record['objectaddresses'] as $address) {
                if (\tx_nclib::isLoopable($address)) {
                    if ($this->hasGoogleMapsEnabled(
                        ) && (!empty($address['address']) || !empty($address['municipality']))
                    ) {
                        $addressesAdded = true;
                        $city = $address['city'];
                        if (empty($city)) {
                        ...
                        $link = $this->controller->getHtmlCompliantLinkToController(
                            false,
                            $this->controller->configModel->get('displayPage'),
                            array('id' => $record['uid']),
                            false
                        ...
                        $this->map->addMarkerByAddress(
                            ...
                            ucfirst(
                                $address['address']
                            ) . ' ' . $address['addressnumber'] . $address['addressnumberadditional'] . '<br />' . $address['zipcode'] . ' ' . $city
                        );
                    }
                }
            }
    ...
    public function getListAll()
    {
        ...
        $subparts['ACTION'] = $this->controller->getLinkToController(
            false,
            false,
            array(
                'activeMonth' => $this->controller->getPiVar('activeMonth'),
        ...
        $content = $this->subpartReplaceRecursive(
            ...
            false,
            true,
            $this->controller->configModel->get('cleanRemainingMarkers')
        );
        return $content;
    }
    ...
    public function getDetailsAll()
    {
        ...
        if ($this->controller->configModel->get('showDetailsFieldList') != false) {
            $permitFields = GeneralUtility::trimExplode(
                ',',
                $this->controller->configModel->get('showDetailsFieldList')
            );
        }
        foreach ($permitFields as $field) {
            $helpText = '';
            ...
            $permit['LABEL_FIELDHEADER_' . strtoupper($field)] = $this->getTranslatedLabel(
                    'label_fieldheader_' . $field
                ) . $helpText;
        }
        ...
        if ($this->controller->permitsModel->isLoaded()) {
            ...
            if (\tx_nclib::isLoopable($record['documents'])) {
                foreach ($record['documents'] as $index => $document) {
                    ...
                    $permit['DOCUMENTS'][$index]['PERMIT_DOCUMENT'] = $this->controller->getLinkToController(
                        $doctype,
                        false,
                        array('id' => $record['uid'], 'doc' => md5($document))
                    );
            ...
            $permit['FIELD_RELATED'] = $this->controller->getURLToFilteredResultPublication(
                array('id' => $record['related'])
            );
        ...
        $subparts['LINK_BACK'] = $this->controller->getHtmlCompliantLinkToController(
            false,
            false,
            array(
                'activeMonth' => $this->controller->getPiVar('activeMonth'),
    ...
    {
        $search = 'document.getElementById("' . $this->map->mapName . '").style.height="' . $this->controller->configModel->get(
                'googleMaps.height'
            ) . 'px";';
        $replace = $search . 'document.getElementById("' . $this->map->mapName . '").style.width="' . $this->controller->configModel->get(
                'googleMaps.width'
            ) . 'px";';
    ...
    public function getDocumentAll()
    {
        ...
        if ($record !== false) {
            ...
            $permit['PERMIT_LINK'] = $this->controller->getHtmlCompliantLinkToController(
                false,
                false,
                array('id' => $record['uid']),
                false
                ...
                if ($iMonth == $aData['iActiveMonth']) {
                    ...
                    if ($iMonth > $aData['iCurrentMonth'] && $aData['bCurrentYearActive']) {
                        ...
                        $aResult['MONTHS'][$iIndex]['MONTH'] =
                            $this->controller->getLinkToFilteredResult(
                                $sMonth,
                                array('activeYear' => $aData['iActiveYear'], 'activeMonth' => $iMonth)
                            );
                    } else {
                        $aResult['MONTHS'][$iIndex]['MONTH'] =
                            $this->controller->getLinkToFilteredResult(
                    ...
                    if ($iWeek > $aData['iCurrentWeek'] && $aData['bCurrentYearActive']) {
                        ...
                        $aResult['WEEKS'][$iIndex]['WEEK'] =
                            $this->controller->getLinkToFilteredResult(
                                $sWeek,
                                array(
                                    'activeYear' => $aData['iActiveYear'],
                                    'activeMonth' => $aData['iActiveMonth'],
                                    'activeWeek' => $iWeek
                                )
                            );
                    } else {
    ...
    function getMonthFilter()
    {
        ...
        if (\tx_nclib::isLoopable($aMonths)) {
            foreach ($aMonths as $iIndex => $iMonth) {
                ...
                {
                    $result['PRODUCT_TYPES'][$index]['TITLE'] = htmlentities(
                        $this->getTranslatedLabel(
                            'label_type_' .
                            ($this->controller->permitsModel->getModelType(
                            ) == \Netcreators\NcgovPermits\Domain\Model\Permit::TYPE_PERMIT ? 'permit' : 'publication')
                        )
                    );
                }
            }
    ...
    function getProductTypeFilter()
    {
        ...
        for ($index = 0; $index < count($productTypes); $index++) {
            ...
            $result['PRODUCT_TYPES'][$index]['SELECTED'] = $this->controller->getPiVar(
                'productType'
            ) == $productTypes[$index] ? ' selected="selected"' : '';
    ...
    function getTermTypeFilter()
    {
        ...
        for ($index = 0; $index < count($termTypes); $index++) {
            ...
            $result['TERM_TYPES'][$index]['SELECTED'] = $this->controller->getPiVar(
                'termType'
            ) == $termTypes[$index] ? ' selected="selected"' : '';
    ...
    function getPhaseFilter()
    {
        ...
        for ($index = 0; $index < count($phases); $index++) {
            $phaseIsChecked = is_array(
                    $selectedPhases
                ) && isset($selectedPhases[$index]) && $selectedPhases[$index] == 1;
            // generate links on the server side instead of submitting a form, in order to be able to create cHashes and have proper caching!
...
//			$result['PHASE_OPTIONS'][$index]['URL'] = $this->controller->htmlEntitiesForQuotes( $this->controller->getURLToFilteredResult(array('phase' => $newPhase)) );
    ...
    {
        $result = array();
        if ($this->controller->configModel->get('regionSearch') || $this->controller->configModel->get(
                'regionStringSearch'
            )
        ) {
    ...
    public function getMetaDataRecord($isDocument = false)
    {
        ...
        if ($this->controller->permitsModel->isPermit() || empty($link)) {
            $link = $this->getDublinCoreMetaData(
                'identifier',
        ...
        $lines[] = $this->getDublinCoreMetaData(
            'informatietype',
            $this->controller->configModel->get('owmsDefaults.informationType')
        );
        ...
        if (!$isDocument) {
            $lines[] = $this->getDublinCoreMetaData(
                'references',
                $this->controller->permitsModel->getField('casereference')
            );
        ...
        $lines[] = $this->getDublinCoreMetaData(
            'description',
            $this->controller->permitsModel->getField('description')
        );
        ...
        $lines[] = $this->getDublinCoreMetaData(
            'organisationType',
            $this->controller->configModel->get('owmsDefaults.organisationType')
        );
        return $lines;
    }
    ...
    {
        $lines = $this->getMetaDataRecord();
        $lines = array_merge_recursive(
            $lines,
            $this->getDublinCoreMetaData(
                'isPartOf',
                \tx_nclib_tsfe_model::getBaseUrl() .
                $this->controller->getLinkToController(
                    false,
...
//			'organisationType' => 	array('OVERHEID.%s', 		'OVERHEID.organisatietype', 	false),
    ...
    protected function getPermitXmlCaseHeader($type, $namespace, $index, $isDocument = false)
    {
        ...
        foreach ($lines as $line) {
            ...
            if ($name == 'dcterms:creator' && $isDocument) {
                $scheme = 'overheid:' . ucfirst(
                        $value = $this->controller->configModel->get('owmsDefaults.creatorType')
                    );
    ...
    public function getPermitXml($index)
    {
        ...
        if ($this->controller->permitsModel->getField('objectreference') != '') {
            $child = $dom->createElement(
                'referentienummer',
                $this->controller->permitsModel->getField('objectreference')
            );
            ...
            foreach ($addresses as $adresIndex => $adres) {
                ...
                if (!empty($addresses[$adresIndex]['zipcode'])) {
                    ...
                    $zip->appendChild(
                        $dom->createElement(
                            'postcode',
                            str_replace(' ', '', strtoupper($addresses[$adresIndex]['zipcode']))
                        )
                    );
                    $zip->appendChild(
