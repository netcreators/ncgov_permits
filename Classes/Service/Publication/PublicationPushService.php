...
class PublicationPushService
{
    ...
    function buildXML()
    {
        ...
        if (!empty($validityEnd)) {
            ...
            $temporalSubpartArray['###TEMPORALEND###'] = $this->substituteMarkerArrayCached(
                $temporalendTemplateCode,
                $temporalendMarkerArray
            );
        }

        $localSubpartArray['###TEMPORAL###'] = $this->substituteMarkerArrayCached(
            $temporalTemplateCode,
            $temporalMarkerArray,
            $temporalSubpartArray
        );
        ...
        if (!empty($this->data['publication'])) {
            ...
            $localSubpartArray['###PUBLICATIEBLOCK###'] = $this->substituteMarkerArrayCached(
                $publicatieTemplateCode,
                $publicatieMarkerArray
            );
        }
        ...
        if (!empty($this->data['termtype_start']) || !empty($this->data['termtype_end'])) {
            ...
            if (!empty($this->data['termtype_start'])) {
                ...
                $termijnSubpartArray['###STARTTERMIJN###'] = $this->substituteMarkerArrayCached(
                    $startTermijnTemplateCode,
                    $startTermijnMarkerArray
                );
            }
            ...
            if (!empty($this->data['termtype_end'])) {
                ...
                $termijnSubpartArray['###EINDTERMIJN###'] = $this->substituteMarkerArrayCached(
                    $eindTermijnTemplateCode,
                    $eindTermijnMarkerArray
                );
            }

            $localSubpartArray['###TERMIJN###'] = $this->substituteMarkerArrayCached(
                $termijnTemplateCode,
                null,
                $termijnSubpartArray
            );
        ...
        if (!empty($this->data['objectreference'])) {
            ...
            $localSubpartArray['###REFERENTIENUMMERBLOCK###'] = $this->substituteMarkerArrayCached(
                $referentienummerTemplateCode,
                $referentienummerMarkerArray
            );
        }
        ...
        foreach ($this->data['addresses'] as $address) {
            ...
            if (!empty($address['zipcode'])) {
                $postcodehuisnummerTemplateCode = HtmlParser::getSubpart(
                    $adresTemplateCode,
                    '###POSTCODEHUISNUMMER###'
                );
                ...
                if (!empty($address['addressnumber'])) {
                    $huisnummerTemplateCode = HtmlParser::getSubpart(
                        $postcodehuisnummerTemplateCode,
                        '###HUISNUMMERBLOCK###'
                    );
                    $huisnummerMarkerArray = array(
                        '###HUISNUMMER###' => $address['addressnumber']
                    );
                    $postcodehuisnummerSubpartArray['###HUISNUMMERBLOCK###'] = $this->substituteMarkerArrayCached(
                        $huisnummerTemplateCode,
                        $huisnummerMarkerArray
                    );
                }

                if (!empty($address['addressnumberadditional'])) {
                    $huisnummertoevoegingTemplateCode = HtmlParser::getSubpart(
                        $postcodehuisnummerTemplateCode,
                        '###HUISNUMMERTOEVOEGINGBLOCK###'
                    );
                    ...
                    $postcodehuisnummerSubpartArray['###HUISNUMMERTOEVOEGING###'] = $this->substituteMarkerArrayCached(
                        $huisnummertoevoegingTemplateCode,
                        $huisnummertoevoegingMarkerArray
                    );
                }

                $adresSubpartArray['###POSTCODEHUISNUMMER###'] = $this->substituteMarkerArrayCached(
                    $postcodehuisnummerTemplateCode,
                    $postcodehuisnummerMarkerArray,
                    $postcodehuisnummerSubpartArray
                );
            }
            ...
            if (!empty($address['municipality'])) {
                ...
                $adresSubpartArray['###GEMEENTEBLOCK###'] = $this->substituteMarkerArrayCached(
                    $gemeenteTemplateCode,
                    $gemeenteMarkerArray
                );
            }
            ...
            if (!empty($address['province'])) {
                ...
                $adresSubpartArray['###PROVINCIEBLOCK###'] = $this->substituteMarkerArrayCached(
                    $provincieTemplateCode,
                    $provincieMarkerArray
                );
            }

            $localSubpartArray['###LOCATIEADRES###'] .= $this->substituteMarkerArrayCached(
                $adresTemplateCode,
                null,
                $adresSubpartArray
            );
        ...
        foreach ($this->data['parcels'] as $parcel) {
            ...
            $localSubpartArray['###LOCATIEPERCEEL###'] .= $this->substituteMarkerArrayCached(
                $perceelTemplateCode,
                $perceelMarkerArray
            );
        }
        ...
        foreach ($this->data['coordinates'] as $coordinates) {
            ...
            $localSubpartArray['###LOCATIECOORDINATEN###'] .= $this->substituteMarkerArrayCached(
                $coordinatenTemplateCode,
                $coordinatenMarkerArray
            );
        }
     ...
     */
    public function substituteMarkerArrayCached(
        $content,
        ...
        asort($aKeys);
        $storeKey = md5(
            'substituteMarkerArrayCached_storeKey:' . serialize(
                array(
                    $content,
                    $aKeys
                )
